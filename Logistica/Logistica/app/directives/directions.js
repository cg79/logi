...
(function() {
  ...
  var directions = function(Attr2Options, $timeout) {
    ...
    var updateRoute = function(renderer, options, mapObj) {
          ...
          if (status == google.maps.DirectionsStatus.OK) {
            ...
            $timeout(function() {
              ...
              for (var i = 0; i < op.length; i++) {
                coordinates.push(new google.maps.LatLng(op[i].A, op[i].F));
              }
              ...
              flightPath.setMap(mapObj.map);
              google.maps.event.addListener(flightPath, 'click', function(clickEvent, a, b) {
    ...
    var linkFunc = function(scope, element, attrs, mapController) {
        ...
        (function(attrName) {
          attrs.$observe(attrName, function(val) {
            if (options[attrName] !== val) { //apply only if changed
              ...
              updateRoute(renderer, options, mapController);
            }
          });
        })(attrName);
      });
      ...
      scope.$on('mapInitialized', function(event, map) {
        updateRoute(renderer, options, map);
      });
    };
